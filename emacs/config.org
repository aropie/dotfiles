...
#+BEGIN_SRC emacs-lisp
  (use-package multi-vterm
    ...
    :general
    ("C-<return>" 'my/dwim-toggle-dedicated-vterm)
    ("C-M-<return>" 'my/switch-dedicated-vterm)
    :init
    ...
    ;; Set per-project via .dir-locals.el. Example:
    ;;   ((nil . ((my/vterm-templates . (("name" . "command")
    ;;                                       ("other" . "other command"))))))
    (defvar my/vterm-templates nil
      "Alist mapping vterm names to commands to run on creation.
    Set per-project via .dir-locals.el.")
    (put 'my/vterm-templates 'safe-local-variable #'listp)

    (defun my/switch-dedicated-vterm ()
      "Switch which vterm buffer is the dedicated one.

    If the selected name matches a template in `my/vterm-templates'
    and no buffer exists for it yet, create it and run the template command."
      (interactive)
      (let* ((vterm-buffers (cl-remove-if-not
                             ...
                             (buffer-list)))
             (existing-names (mapcar (lambda (buf)
                                       (let ((name (buffer-name buf)))
                                         (if (string-match "^\\*vterminal - \\(.*\\)\\*$" name)
                                             (match-string 1 name)
                                           name)))
                                     vterm-buffers))
             (template-names (mapcar #'car my/vterm-templates))
             (candidates (delete-dups (append existing-names template-names)))
             (selected-name (completing-read "Select vterm buffer: " candidates nil nil))
             (buffer-exists-p (get-buffer (format "*vterminal - %s*" selected-name)))
             (template-command (cdr (assoc selected-name my/vterm-templates)))
             (dedicated-buffer-window (get-buffer-window multi-vterm-dedicated-buffer)))
        (when dedicated-buffer-window
          (multi-vterm-dedicated-close))
        (setq multi-vterm-dedicated-buffer-name selected-name)
        (setq multi-vterm-dedicated-buffer nil)
        (multi-vterm-dedicated-open)
        (when (and template-command (not buffer-exists-p))
          (with-current-buffer multi-vterm-dedicated-buffer
            (vterm-send-string template-command)
            (vterm-send-return)))))

    (defun my/dwim-toggle-dedicated-vterm ()
