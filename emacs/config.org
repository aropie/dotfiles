...
#+END_SRC
**
** Prefer as much as possible vertical splits
Taken from [[https://emacs.stackexchange.com/questions/28496/magit-status-always-split-vertically][here]]
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  (my-leader
    :infix "o"
    "s" '(aropie/get-scratch-buffer :which-key "Scratch"))
#+END_SRC
** Which-key
...
** Fireplace
   #+BEGIN_SRC emacs-lisp
     (use-package fireplace
       :hook (fireplace-mode . evil-emacs-state)
       :custom (fireplace-smoke-on t)
       :init
       (defun run-fireplace ()
         (command-execute 'fireplace))
       (run-with-idle-timer 600 t 'run-fireplace))
   #+END_SRC
** Highlight TODOs
#+BEGIN_SRC emacs-lisp
  (use-package hl-todo
