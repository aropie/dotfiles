...
#+BEGIN_SRC emacs-lisp
  (use-package multi-vterm
    ...
    ;;   ((nil . ((aropie/vterm-templates . (("name" . "command")
    ;;                                       ("other" . "other command"))))))
    (defvar aropie/vterm-templates nil
      "Alist mapping vterm names to commands to run on creation.
    Set per-project via .dir-locals.el.")
    (put 'aropie/vterm-templates 'safe-local-variable #'listp)

    (defun aropie/switch-dedicated-vterm ()
      "Switch which vterm buffer is the dedicated one.

    If the selected name matches a template in `aropie/vterm-templates'
    and no buffer exists for it yet, create it and run the template command."
      ...
      (let* ((vterm-buffers (cl-remove-if-not
             ...
             (existing-names (mapcar (lambda (buf)
                                       (let ((name (buffer-name buf)))
                                         (if (string-match "^\\*vterminal - \\(.*\\)\\*$" name)
                                           ...
                                           name)))
                                     vterm-buffers))
             (template-names (mapcar #'car aropie/vterm-templates))
             ...
             (template-command (cdr (assoc selected-name aropie/vterm-templates)))
...
#+BEGIN_SRC emacs-lisp
  (use-package lsp-mode
    ...
    (defun aropie/lsp-setup-xref-fallback ()
      "Ensure dumb-jump is available as xref fallback after LSP."
      (add-hook 'xref-backend-functions #'dumb-jump-xref-activate nil t))

    (add-hook 'lsp-after-open-hook #'aropie/lsp-setup-xref-fallback))
...
#+BEGIN_SRC emacs-lisp
  (defvar aropie/django-test-command
    "Command template for running Django tests. %s is replaced with the test path.
    Set per-project via .dir-locals.el."
      (concat "django" "manage.py" "test %s'"))
  (put 'aropie/django-test-command 'safe-local-variable #'stringp)

  (defun aropie/django-test--module-path ()
  ...
  (defun aropie/django-test--enclosing-node (type)
  ...
  (defun aropie/django-test--node-name (node)
  ...
  (defun aropie/django-test--send (test-path)
    "Run TEST-PATH in a compilation buffer."
    (compile (format aropie/django-test-command test-path)))


  (defun aropie/django-test-at-point ()
    "Run the Django test at point (method > class > module)."
    (interactive)
    (let* ((module (aropie/django-test--module-path))
           (class-node (aropie/django-test--enclosing-node 'class_definition))
           (class-name (when class-node (aropie/django-test--node-name
  									   class-node)))
           (func-node (aropie/django-test--enclosing-node 'function_definition))
           (func-name (when func-node (aropie/django-test--node-name
  									 func-node)))
           (method-name (when (and func-name (string-prefix-p "test_"
  															func-name))
                          func-name)))
      (aropie/django-test--send
       (concat module
               (when class-name (concat "." class-name))
               (when method-name (concat "." method-name))))))

  (defun aropie/django-test-class ()
  ...
  (defun aropie/django-test-module ()
    "Run all tests in the current module."
    (interactive)
    (aropie/django-test--send (aropie/django-test--module-path)))

  (my-local-leader
      :keymaps 'python-ts-mode-map
      :state 'normal
      "t" '(:ignore t :which-key "Test")
      "tt" '("Test at point" . aropie/django-test-at-point)
      "tc" '("Test class" . aropie/django-test-class)
      "tm" '("Test module" . aropie/django-test-module))
#+END_SRC
*** SVG tags
#+BEGIN_SRC emacs-lisp
  (add-hook 'python-ts-mode-hook
